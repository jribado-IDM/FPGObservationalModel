...
class StaticCredentialPrompt(CredentialPrompt):
    """
    A script to log in to COMPS using provided username and password arguments.
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--comps_url', default='https://comps.idmod.org', help='comps url')
    parser.add_argument('-u', '--username', help='enter username')
    parser.add_argument('-p', '--password', help='enter password')

    args = parser.parse_args()

    Client.login(args.comps_url, StaticCredentialPrompt(comps_url=args.comps_url, username=args.username,
